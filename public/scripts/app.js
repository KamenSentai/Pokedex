...
if ($containerMap) {
  //Get elements
  var $horizontal = $containerMap.querySelector('.horizontal');
  var $vertical = $containerMap.querySelector('.vertical');
  ...
  var setStyles = function setStyles() {
    var realSize = windowWidth / windowHeight <= MAP_RATIO ? windowWidth : windowHeight;
    ...
    var leftOffset = (windowWidth - tileSize.x * MAP_COL) / 2;
    var topOffset = (windowHeight - tileSize.y * MAP_ROW) / 2;
  ...
  // Initialize map size
  setTimeout(function () {
    resizeImage(windowWidth, windowHeight, setStyles) - document.body.classList.remove('fade');
  }, 250);
